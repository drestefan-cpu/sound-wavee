...
const AdminPanel = ({ onClose }: AdminPanelProps) => {
  ...
  const addTagline = async () => {
    ...
    const { data } = await supabase
      .from("taglines")
      .insert(newTagline as any)
      .select()
      .single();
    if (data) {
      setTaglines((prev) => [...prev, data]);
      setNewTagline({ text: "", category: "general", weight: 5 });
    }
  };
  ...
  const addPick = async () => {
    ...
    const { data } = await supabase
      .from("plai_picks")
      .insert({ ...newPick, position } as any)
      .select()
      .single();
    if (data) {
      setPicks((prev) => [...prev, data]);
      setNewPick({ spotify_track_id: "", title: "", artist: "", album: "", note: "" });
  ...
  return (
    <div
      className={`fixed inset-0 z-[60] flex justify-center bg-black/60 backdrop-blur-sm ${authed ? "items-end" : "items-center"}`}
      onClick={onClose}
    >
      <div
        className={`w-full max-w-lg bg-card overflow-y-auto ${authed ? "border-t border-border rounded-t-2xl max-h-[85vh]" : "border border-border rounded-2xl mx-4"}`}
              ...
              <div>
                {taglinesError ? (
                  <p className="text-xs text-muted-foreground text-center py-8">
                    taglines table not found — run the SQL migration first
                  </p>
                ) : (
                  <>
                    <p className="text-xs text-muted-foreground mb-3">
                      {taglines.length} taglines · {activeCount} active
                    </p>
                    ...
                    <div className="flex gap-2 mb-4 items-center">
                      ...
                      />
                      <select
                        value={newTagline.category}
                        onChange={(e) => setNewTagline((p) => ({ ...p, category: e.target.value }))}
                      ...
                      <input
                        ...
                        min={1}
                        max={10}
                        value={newTagline.weight}
                        onChange={(e) => setNewTagline((p) => ({ ...p, weight: Number(e.target.value) }))}
                        className="w-12 px-2 py-1.5 text-xs text-center"
                        style={inputStyle}
                      />
                      <button
                        onClick={addTagline}
                        className="rounded-full bg-primary px-3 py-1.5 text-[10px] text-primary-foreground font-medium flex-shrink-0"
                      >
                        Add
                      </button>
                    </div>
                    ...
                    <div className="rounded-xl border border-border overflow-hidden">
                      {taglines.map((t, idx) => (
                        <div
                          ...
                          style={{ height: 40, background: idx % 2 === 0 ? "#0F1520" : "#080B12" }}
                        >
                          ...
                          {editingId === t.id ? (
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter") saveTaglineText(t.id);
                              }}
                          ...
                          <select
                            value={t.category}
                            onChange={(e) => updateTaglineCategory(t.id, e.target.value)}
                          ...
                          >
                            <div
                              className="absolute top-0.5 rounded-full w-3 h-3 bg-white transition-all"
                          ...
                          {deleteConfirm === t.id ? (
                            <div className="flex gap-1 flex-shrink-0">
                              <button onClick={() => deleteTagline(t.id)} className="text-[9px] text-red-400">
                                yes
                              </button>
                              <button
              ...
              <div className="space-y-2">
                ...
                {showAddPick && (
                  <div className="rounded-xl border border-primary/30 bg-background p-3 space-y-2 mb-3">
                    <Input
                      value={newPick.spotify_track_id}
                      onChange={(e) => setNewPick((p) => ({ ...p, spotify_track_id: e.target.value }))}
                      placeholder="spotify_track_id"
                      className="bg-card border-border text-sm"
                    />
                    <Input
                      value={newPick.title}
                      onChange={(e) => setNewPick((p) => ({ ...p, title: e.target.value }))}
                      placeholder="title"
                      className="bg-card border-border text-sm"
                    />
                    <Input
                      value={newPick.artist}
                      onChange={(e) => setNewPick((p) => ({ ...p, artist: e.target.value }))}
                      placeholder="artist"
                      className="bg-card border-border text-sm"
                    />
                    <Input
                      value={newPick.album}
                      onChange={(e) => setNewPick((p) => ({ ...p, album: e.target.value }))}
                      placeholder="album"
                      className="bg-card border-border text-sm"
                    />
                    <Input
                      value={newPick.note}
                      onChange={(e) => setNewPick((p) => ({ ...p, note: e.target.value }))}
                      placeholder="note"
                      className="bg-card border-border text-sm"
                    />
                    <button
                ...
                {picks.map((p) => (
                  ...
                  >
                    ...
                    {p.album_art_url ? (
                      ...
                      <div className="h-8 w-8 rounded bg-card border border-border flex items-center justify-center text-[10px] text-muted-foreground flex-shrink-0">
                        ♪
                      </div>
                    )}
                    ...
                    <button
                      onClick={() => togglePickActive(p.id, p.active)}
                      className={`rounded-full px-1.5 py-0.5 text-[9px] ${p.active ? "bg-green-500/20 text-green-400" : "bg-muted text-muted-foreground"}`}
                    >
                      {p.active ? "on" : "off"}
                    </button>
                    <button onClick={() => deletePick(p.id)} className="text-muted-foreground hover:text-destructive">
                      <Trash2 className="h-3 w-3" />
                    </button>
                  </div>
                ))}
